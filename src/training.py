...
def train_validate_survival_model(model, train_dataloader, val_dataloader, loss_function, optimizer, num_epochs, patience, print_every=5):
    """
    Trains and validates a PyTorch model specifically designed for datasets involving recurrent events and terminal events.
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            optimizer.zero_grad()
            pred_y = model(batch_x).squeeze(-1)
            loss = loss_function(pred_y, batch_t.long(), batch_d.long())
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
        train_loss /= len(train_dataloader)
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                batch_x, batch_t, batch_e, batch_d = batch
                pred_y = model(batch_x).squeeze(-1)
                val_loss += loss_function(pred_y, batch_t.long(), batch_d.long()).item()
        val_loss /= len(val_dataloader)
...
def train_validate_recurrent_model(model, train_dataloader, val_dataloader, loss_function, optimizer, num_epochs, patience, print_every=5):
    """
    Trains and validates a PyTorch model specifically designed for datasets involving recurrent events and terminal events.
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            optimizer.zero_grad()
            pred_y = model(batch_x).squeeze(-1)
            loss = loss_function(pred_y, batch_t.long(), batch_e.long())
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
        train_loss /= len(train_dataloader)
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                batch_x, batch_t, batch_e, batch_d = batch
                pred_y = model(batch_x).squeeze(-1)
                val_loss += loss_function(pred_y, batch_t.long(), batch_e.long()).item()
        val_loss /= len(val_dataloader)
